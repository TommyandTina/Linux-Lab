...
int main() {
    int width = 3856; // replace by image width
    int height = 1964; // replace by image height
    int bpp = 8; // bits per pixel for: NV16 format - 8, RGB24 - 24
	int align = 256; //must align with __ value
	int num_of_planar = 2;//nv16 has 2, rgb24 has 3
    // Calculate stride, align-1 is to round up stride to align with 256 
    int stride = ((width + align-1) / align)*align;

    // Calculate size (in bytes) and print out stride and size - EX2_1
    int size = stride * height * num_of_planar;
...
//EX2_2 : 
    ...
    // Open the NV16 file
    FILE *file = fopen("imageprocessing_requirement1_nv16.yuv", "rb");
    if (file == NULL) {
    ...
    for (int i = 0; i < height; ++i) {
        if (fread(image + i * stride, 1, width, file) != width) {
    ...
    file = fopen("imageprocessing_requirement1_nv16_padded.yuv", "wb");
    if (file == NULL) {
