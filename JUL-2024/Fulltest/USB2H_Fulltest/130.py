...
if __name__ == '__main__':
    ...
    for CN in range(0,4):
        if USB2H_LIST[CN] is not None:
            plug_in_usb(a)
            mountpoint = usb2h_fulltest.wait_usb2h(a, USB2H_LIST[CN], True)
            mnt = "/mnt/usb{}".format(count)
            a.buff = ""
            a.send("mount /dev/{} {}".format(mountpoint,mnt))
            ...
            a.send("cp testdata {}/testdata_from_nfs".format(mnt))
            a.send("cp {}/testdata_from_nfs testdata_from_usb".format(mnt))
            a.send("sync")
