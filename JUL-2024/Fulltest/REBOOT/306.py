...
if __name__ == '__main__':
    ...
    for i in range(0, len(usb2h_fulltest.USB2H_LIST)):
        ...
        a.send("dd if=/dev/{} of=/dev/null bs=1M count=10 iflag=direct & top -b -d 1 -n 10 > aaa.txt".format(mountpoint))
        a.send("export pid=$!")
        a.send("echo $pid")
        a.send("wait $pid")
        a.send("grep $pid aaa.txt")
        a.send("dd if=/dev/zero of=/dev/{} bs=1M count=10 oflag=direct & top -b -d 1 -n 10 > bbb.txt".format(mountpoint)) 
        a.send("export pid=$!")
        a.send("echo $pid")
        a.send("wait $pid")
