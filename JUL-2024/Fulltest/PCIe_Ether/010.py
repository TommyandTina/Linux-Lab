 ...
 #   if (a.buff.find('{} packets received, 0% packet loss'.format(time))!=-1):
  #      return False
   ...
   # a.send=('ifconfig enp1s0 up 192.168.10.14')
    ...
    a.send('route add default gw {} enp1s0'.format(config.DEFAULT_GW)) 
    pcie.ping_ip(a,config.IP_ADDR,1)
    pcie.ping_ip(a,config.IP_ADDR,100)
    pcie.ping_ip(a,config.IP_ADDR,127)
    pcie.ping_ip(a,config.IP_ADDR,1000)
    pcie.ping_ip(a,config.IP_ADDR,15000)
