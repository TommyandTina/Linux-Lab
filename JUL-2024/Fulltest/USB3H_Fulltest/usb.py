...
def list_usb_partition_usb3(a,channel):
    if (channel == 'usb3'):
        a.send("find /sys/devices/platform/soc/{}/ -name 'sd*'".format(USB3_INTERRUPT),0.005)
        return True
    else:
...
def mountpoint_usb3(a,channel):
    a.buff=""
    channel = channel.lower()    
    
    if not list_usb_partition_usb3(a, channel):
        return None 
...
#Insert = False, not require insert device after login
def wait_usb3h(a,insert=False):
    if insert:
        while (a.buff.find("Attached SCSI removable disk") == -1):
            sleep(0.1)
        time.sleep(1)
    mountpoint = mountpoint_usb3(a, channel = "usb3")
    if mountpoint == None:
        print_result.func_fail(a)
    return mountpoint

def wait_usb3h_hub(a,insert=False):
    if insert:
        while (a.buff.find("Attached SCSI removable disk") == -1):
            sleep(0.1)
        time.sleep(1)
    mountpoint = mountpoint_usb_hub(a, channel = "usb3")
    if mountpoint == None:
        print_result.func_fail(a)
    return mountpoint
