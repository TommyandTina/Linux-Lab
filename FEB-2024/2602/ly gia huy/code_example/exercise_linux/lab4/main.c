...
static int __init mychardev_init(void)
{
    ...
    printk("MYCHARDEV - Major_Number: %d\n", dev_major);
...
static ssize_t mychardev_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
    ...
    remaining_bytes = datalen - *offset;
...
static ssize_t mychardev_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
    size_t maxdatalen = sizeof(buffer), ncopied;

    printk(KERN_INFO "Writing device: %d\n", MINOR(file->f_path.dentry->d_inode->i_rdev));
    printk(KERN_INFO "Write data: %s", buf);

    if (count < maxdatalen) {
        maxdatalen = count;
    }

    ncopied = copy_from_user(buffer, buf, maxdatalen);
    return count;
}

MODULE_LICENSE("GPL");
MODULE_AUTHOR("LGH huy.ly-gia@banvien.com.vn");
