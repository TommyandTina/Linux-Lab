...
static const struct ImpDrvVALUE_TO_STRING_TABLE m_tableNames[] =
{
    { TEST_VALIDATOR_RETCODE, ENUM_TO_STRING(E_SUCCESS) },
    { TEST_VALIDATOR_RETCODE, ENUM_TO_STRING(E_FAILURE) },
    { TEST_VALIDATOR_RETCODE, ENUM_TO_STRING(E_VALIDATION_FAILURE) },
    NULL
};

static void TEST_ImpValueToString(uint64_t validator, int64_t ret, char *string)
{
    for (int i = 0; i < ARRAY_COUNT(m_tableNames); i++)
    {
        if (validator & m_tableNames[i].validator)
        {
            if (ret == m_tableNames[i].data.ret)
            {
                strcpy(string, m_tableNames[i].data.name);
                return;
            }
            else
            {
